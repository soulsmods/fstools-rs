...
type FLVERReader = std::fs::File;

pub trait FLVERPartReader {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error>
    where
        Self: Sized;
}
...
impl FLVER {
    pub fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        ...
        let bounding_box_min = FLVERVector3::from_reader(r)?;
        let bounding_box_max = FLVERVector3::from_reader(r)?;
        ...
        let dummies = read_vec::<FLVERDummy>(r, dummy_count as usize)?;
        let materials = read_vec::<FLVERMaterial>(r, material_count as usize)?;
        let bones = read_vec::<FLVERBone>(r, bone_count as usize)?;
        let meshes = read_vec::<FLVERMesh>(r, mesh_count as usize)?;
        let face_sets = read_vec::<FLVERFaceSet>(r, face_set_count as usize)?;
        let vertex_buffers = read_vec::<FLVERVertexBuffer>(r, vertex_buffer_count as usize)?;
        let buffer_layouts = read_vec::<FLVERBufferLayout>(r, buffer_layout_count as usize)?;
        let textures = read_vec::<FLVERTexture>(r, texture_count as usize)?;
...
impl FLVERPartReader for FLVERDummy {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        Ok(Self {
            position: FLVERVector3::from_reader(r)?,
            color: FLVERColor::from_reader(r)?,
            forward: FLVERVector3::from_reader(r)?,
            reference_id: r.read_u16::<LE>()?,
            parent_bone_index: r.read_u16::<LE>()?,
            upward: FLVERVector3::from_reader(r)?,
...
impl FLVERPartReader for FLVERBone {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        let translation = FLVERVector3::from_reader(r)?;
        let name_offset = r.read_u32::<LE>()?;

        let rotation = FLVERVector3::from_reader(r)?;
        let parent_index = r.read_u16::<LE>()?;
        let child_index = r.read_u16::<LE>()?;
        let scale = FLVERVector3::from_reader(r)?;
...
impl FLVERPartReader for FLVERMesh {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        ...
        let bone_indices = read_vec::<u32>(r, bone_count as usize)?;

        r.seek(SeekFrom::Start(face_set_offset as u64))?;
        let face_set_indices = read_vec::<u32>(r, face_set_count as usize)?;

        r.seek(SeekFrom::Start(vertex_buffer_offset as u64))?;
        let vertex_buffer_indices = read_vec::<u32>(r, vertex_buffer_count as usize)?;
...
impl FLVERPartReader for FLVERFaceSet {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        ...
        let current = r.seek(SeekFrom::Current(0))?;
        r.seek(SeekFrom::Start(index_offset as u64 + 194800))?;
        let indices = match index_size {
            0 => FLVERFaceSetIndices::Byte0,
            8 => FLVERFaceSetIndices::Byte1(
                read_vec::<u8>(r, index_count as usize)?
            ),
            16 => FLVERFaceSetIndices::Byte2(
                read_vec::<u16>(r, index_count as usize)?
            ),
            32 => FLVERFaceSetIndices::Byte4(
                read_vec::<u32>(r, index_count as usize)?
            ),
...
impl FLVERPartReader for FLVERBufferLayout {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        ...
        let members = read_vec::<FLVERBufferLayoutMember>(r, member_count as usize)?;
...
impl From<u32> for FLVERLayoutType {
    fn from(value: u32) -> Self {
        match value {
            0x1 => Self::Float2,
            0x2 => Self::Float3,
...
impl From<u32> for FLVERLayoutSemantic {
    fn from(value: u32) -> Self {
        match value {
            0x0 => Self::Position,
...
pub struct FLVERBufferLayoutMember {
    ...
    pub layout_type: FLVERLayoutType,
    pub layout_semantic: FLVERLayoutSemantic,
    pub index: u32,
}
...
impl FLVERPartReader for FLVERBufferLayoutMember {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        Ok(Self {
            ...
            layout_type: r.read_u32::<LE>()?.into(),
            layout_semantic: r.read_u32::<LE>()?.into(),
            index: r.read_u32::<LE>()?,
        })
    }
}
...
impl FLVERPartReader for FLVERTexture {
    fn from_reader(r: &mut FLVERReader) -> Result<Self, io::Error> {
        ...
        let scale = FLVERVector2::from_reader(r)?;
        let unk10 = r.read_u8()?;
...
fn read_vec<T: FLVERPartReader>(r: &mut FLVERReader, count: usize) -> Result<Vec<T>, io::Error> {
    let mut results = Vec::new();
    for _ in 0..count {
        results.push(T::from_reader(r)?);
    }

    Ok(results)
}

fn read_utf16(r: &mut FLVERReader) -> Result<String, io::Error> {
