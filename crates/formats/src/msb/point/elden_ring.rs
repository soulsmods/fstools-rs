...
impl PointType {
    pub fn variants() -> Vec<(PointType, &'static str)> {
        vec![
            ...
            (PointType::InvasionPoint, "InvasionPoint"),
            ...
            (PointType::Message, "Message"),
            (
                PointType::EnvironmentMapEffectBox,
                "EnvironmentMapEffectBox",
            ),
            (PointType::WindArea, "WindArea"),
            (PointType::Connection, "Connection"),
            (PointType::PatrolRoute22, "PatrolRoute22"),
            (PointType::BuddySummonPoint, "BuddySummonPoint"),
            ...
            (PointType::NavmeshCutting, "NavmeshCutting"),
            (PointType::MapNameOverride, "MapNameOverride"),
            (PointType::MountJumpFall, "MountJumpFall"),
            (PointType::HorseProhibition, "HorseProhibition"),
        ]
    }
}
...
impl From<i32> for PointType {
    fn from(v: i32) -> Self {
        match v {
            ...
            18 => PointType::WindArea,
            21 => PointType::Connection,
            22 => PointType::PatrolRoute22,
            26 => PointType::BuddySummonPoint,
            ...
            36 => PointType::AutoDrawGroupPoint,
            37 => PointType::GroupDefeatReward,
            38 => PointType::MapPointDiscoveryOverride,
            39 => PointType::MapPointParticipationOverride,
            40 => PointType::Hitset,
            41 => PointType::NpcArea,
            42 => PointType::WeatherCreateAssetPoint,
            43 => PointType::PlayArea,
            44 => PointType::EnvironmentMapOutput,
            46 => PointType::MountJump,
            48 => PointType::Dummy,
            49 => PointType::FallPreventionRemoval,
            50 => PointType::NavmeshCutting,
            51 => PointType::MapNameOverride,
            52 => PointType::MountJumpFall,
            53 => PointType::HorseProhibition,
            _ => PointType::Unknown,
        }
    }
}
...
impl<'a> PointData<'a> {
    pub fn from_type_and_slice(point_type_id: i32, data: &'a [u8]) -> Result<Self, MsbError> {
        ...
        Ok(match point_type {
            ...
            PointType::InvasionPoint => Self::InvasionPoint(
            ...
            PointType::Message => {
                Self::Message(FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?)
            }
            ...
            PointType::WindArea => Self::WindArea,

            PointType::Connection => {
                Self::Connection(FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?)
            }

            PointType::PatrolRoute22 => Self::PatrolRoute22(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),

            PointType::BuddySummonPoint => Self::BuddySummonPoint(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),
            ...
            PointType::AutoDrawGroupPoint => Self::AutoDrawGroupPoint(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),
            ...
            PointType::MapPointDiscoveryOverride => Self::MapPointDiscoveryOverride,

            PointType::MapPointParticipationOverride => Self::MapPointParticipationOverride,
            ...
            PointType::NpcArea => {
                Self::NpcArea(FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?)
            }
            ...
            PointType::PlayArea => {
                Self::PlayArea(FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?)
            }
            ...
            PointType::NavmeshCutting => Self::NavmeshCutting(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),

            PointType::MapNameOverride => Self::MapNameOverride(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),

            PointType::MountJumpFall => Self::MountJumpFall(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),

            PointType::HorseProhibition => Self::HorseProhibition(
                FromBytes::ref_from_prefix(data).ok_or(MsbError::UnalignedValue)?,
            ),
